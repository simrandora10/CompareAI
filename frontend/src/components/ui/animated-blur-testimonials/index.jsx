...
function AnimatedBlurTestimonials({
  ...
  return (
    <MotionConfig transition={{ type: "spring", duration: _duration, bounce: 0 }}>
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: data.length }).map((_, i) => (
              <motion.button
                ...
                className={cn("bg-d-fg/10 h-[6px] cursor-pointer overflow-hidden rounded-xl")}>
                {count === i && (
                  <motion.div
...
function Avatar({
  ...
  return (
    <div
      className="ring-d-bg relative flex size-9 cursor-pointer items-center justify-center overflow-hidden rounded-full bg-white/10 ring-2 backdrop-blur-xl">
      {src ? (
        <img
          src={src}
          draggable={false}
